...
def word_embedding(all_ans_prepro, ticket_ques_prepro, all_docs_prepro):

    #ALL ANSWERS
    # checking if embedding model already exists
    exists = os.path.isfile('embedding/models/word2vec_ans.model')
    if exists:
        print('Word2vec all_ans embedding model already existing')
    ...
    if exists:
        print('Word2vec ticket_ques embedding model already existing')
    else:
    ...
    if exists:
        print('Word2vec all_docs embedding model already existing')
    else:
        # not checking if already exists because if the first doesn't this won't either
        print('Training word2vec on all documents')
